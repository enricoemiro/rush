...
void Player::jump(const Coordinate &next,
                  bool &has_jumped) {
  const char ch_1 =
      mvwinch(map_window, next.y, this->current.x) & A_CHARTEXT;

  const char ch_2 =
      mvwinch(map_window, next.y - 1, this->current.x) & A_CHARTEXT;

  if (ch_1 == '=' || ch_1 == 'X') has_jumped = true;
  if (ch_1 == '=' && ch_2 == ' ') this->current.y = next.y - 1;
}
...
void Player::move(int key_pressed) {
  ...
  if (key_pressed == KEY_RIGHT ||
    ...
    const char ch_1 = mvwinch(this->map_window, this->current.y - 1, this->current.x) & A_CHARTEXT;
    const char ch_2 = mvwinch(this->map_window, this->current.y, this->current.x) & A_CHARTEXT;
    const char ch_3 = mvwinch(this->map_window, this->previous.y - 1, this->previous.x) & A_CHARTEXT;
